...
class Hdfs(object):
    ...
    def copy_within_fs(self, source, target):
        ...
        self.hdfs.copy_to_local(Hdfs.HOME_DIR + source, _omfDir + "/tmp/" + source)
        try:
            print "HDFS: Copy within fs: copying from local... from: " + Hdfs.HOME_DIR + target + " to: " + _omfDir + "/tmp/" + source
            self.hdfs.copy_from_local(_omfDir + "/tmp/" + source, Hdfs.HOME_DIR + target)
        except pyhdfs.HdfsFileAlreadyExistsException:
    ...
    def export_to_hdfs(self, directory, file_to_export):
        ...
        self.hdfs.copy_from_local(file_to_export.filename, Hdfs.HOME_DIR + directory + "/" + file_to_export.filename, overwrite=True)
        return True
    ...
    def export_local_to_hdfs(self, directory, file_to_export):
        ...
        self.hdfs.copy_from_local(file_to_export, Hdfs.HOME_DIR + directory + "/" + filename, overwrite=True)
        return True
    ...
    def import_files_to_hdfs(self, local_directory, hdfs_directory):
        ...
        for f in os.listdir(local_directory):
            self.export_local_to_hdfs(hdfs_directory, local_directory + '/' + f)
        return True


    def recursive_import_to_hdfs(self, start_dir):
        self.create_dir(start_dir)
        for f in os.listdir(_omfDir + "/" + start_dir):
            if os.path.isdir(_omfDir + "/" + start_dir + "/" + f):
                self.create_dir(start_dir + "/" + f)
                self.recursive_import_to_hdfs(start_dir + "/" + f)
            else:
                self.export_local_to_hdfs(start_dir, _omfDir + "/" + start_dir + "/" + f)
        return True
