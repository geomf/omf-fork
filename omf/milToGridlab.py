...
def convert(stdString, seqString):
    ...
    logger.info('Converting Milsoft format to Gridlab')
    ...
    def obConvert(objectList):
        ...
        def convertGenerator(genList):
            ...
            generator['Gen_status'] = (
                'OFFLINE' if genList[26] == '1' else 'ONLINE')
            ...
            if consumer['load_class'] == 'C':
                ...
                if load_mix is None:
                    ...
                    phases = consumer['phases'] + \
                        ('D' if len(consList[2]) >= 2 else '')
                    consumer['phases'] = phases

            # MAYBEFIX: support kVars.
            consumer['constant_power_A'] = str(float(consList[12]) * 1000) + ('+' if float(
                consList[15]) >= 0.0 else '-') + str(abs(float(consList[15]) * 1000)) + 'j'
            consumer['constant_power_B'] = str(float(consList[13]) * 1000) + ('+' if float(
                consList[16]) >= 0.0 else '-') + str(abs(float(consList[16]) * 1000)) + 'j'
            consumer['constant_power_C'] = str(float(consList[14]) * 1000) + ('+' if float(
                consList[17]) >= 0.0 else '-') + str(abs(float(consList[17]) * 1000)) + 'j'
        ...
        def convertNode(nodeList):
            ...
            # this node is actually a load
            if nodeList[15] != '0' or nodeList[16] != '0' or nodeList[17] != '0' or nodeList[18] != '0' or nodeList[19] != '0' or nodeList[20] != '0':
                node['object'] = 'load'
                node['constant_power_A'] = str(float(nodeList[15]) * 1000) + ('+' if float(
                    nodeList[18]) >= 0.0 else '-') + str(abs(float(nodeList[18]) * 1000)) + 'j'
                node['constant_power_B'] = str(float(nodeList[16]) * 1000) + ('+' if float(
                    nodeList[19]) >= 0.0 else '-') + str(abs(float(nodeList[19]) * 1000)) + 'j'
                node['constant_power_C'] = str(float(nodeList[17]) * 1000) + ('+' if float(
                    nodeList[20]) >= 0.0 else '-') + str(abs(float(nodeList[20]) * 1000)) + 'j'

                # setting all nodes with loads on them as commercial loads as
                # we can't know the load classification. If they want the load
                # to be residential then they need to classify the node as a
                # consumer object in the windmil model.
                node['load_class'] = 'C'
            return node
        ...
        def convertOhLine(ohLineList):
            ...
            if load_mix is None or load_mix[5] == 'W':  # Wye connected line
                overhead['phases'] = ohLineList[
                    2] + ('N' if ohLineList[33] == '1' else '')
            ...
            overhead['length'] = (
                '10' if float(ohLineList[12]) < 10 else ohLineList[12])
            ...
            if construction_stats is None:
                # Couldn't find construction object. Using defalut phase
                # distances. Distances are from IEEE 4 Node
                Dab = 2.5
                Dac = 7.0
                Dan = 5.656854
                Dbc = 4.5
                ...
                Dab = math.sqrt(((float(construction_stats[19]) - float(construction_stats[20])) * (float(construction_stats[19]) - float(construction_stats[20]))) + (
                    (float(construction_stats[23]) - float(construction_stats[24])) * (float(construction_stats[23]) - float(construction_stats[24])))) / 12
                Dac = math.sqrt(((float(construction_stats[19]) - float(construction_stats[21])) * (float(construction_stats[19]) - float(construction_stats[21]))) + (
                    (float(construction_stats[23]) - float(construction_stats[25])) * (float(construction_stats[23]) - float(construction_stats[25])))) / 12
                Dan = math.sqrt(((float(construction_stats[19]) - float(construction_stats[22])) * (float(construction_stats[19]) - float(construction_stats[22]))) + (
                    (float(construction_stats[23]) - float(construction_stats[26])) * (float(construction_stats[23]) - float(construction_stats[26])))) / 12
                Dbc = math.sqrt(((float(construction_stats[20]) - float(construction_stats[21])) * (float(construction_stats[20]) - float(construction_stats[21]))) + (
                    (float(construction_stats[24]) - float(construction_stats[25])) * (float(construction_stats[24]) - float(construction_stats[25])))) / 12
                Dbn = math.sqrt(((float(construction_stats[20]) - float(construction_stats[22])) * (float(construction_stats[20]) - float(construction_stats[22]))) + (
                    (float(construction_stats[24]) - float(construction_stats[26])) * (float(construction_stats[24]) - float(construction_stats[26])))) / 12
                Dcn = math.sqrt(((float(construction_stats[21]) - float(construction_stats[22])) * (float(construction_stats[21]) - float(construction_stats[22]))) + (
                    (float(construction_stats[25]) - float(construction_stats[26])) * (float(construction_stats[25]) - float(construction_stats[26])))) / 12
            # Add distances to dictionary when appropriate
            if 'A' in overhead['phases'] and 'B' in overhead['phases']:
                if Dab > 0:
                    overhead[
                        myIndex + 1][myIndex + 2]['distance_AB'] = '{:0.6f}'.format(Dab)
                else:
                    overhead[
                        myIndex + 1][myIndex + 2]['distance_AB'] = '{:0.6f}'.format(2.5)
            else:
                overhead[
                    myIndex + 1][myIndex + 2]['distance_AB'] = '{:0.6f}'.format(0.0)
            if 'A' in overhead['phases'] and 'C' in overhead['phases']:
                if Dac > 0:
                    overhead[
                        myIndex + 1][myIndex + 2]['distance_AC'] = '{:0.6f}'.format(Dac)
                else:
                    overhead[
                        myIndex + 1][myIndex + 2]['distance_AC'] = '{:0.6f}'.format(7.0)
            else:
                overhead[
                    myIndex + 1][myIndex + 2]['distance_AC'] = '{:0.6f}'.format(0.0)
            if 'A' in overhead['phases'] and 'N' in overhead['phases']:
                if Dan > 0:
                    overhead[
                        myIndex + 1][myIndex + 2]['distance_AN'] = '{:0.6f}'.format(Dan)
                else:
                    overhead[
                        myIndex + 1][myIndex + 2]['distance_AN'] = '{:0.6f}'.format(5.656854)
            else:
                overhead[
                    myIndex + 1][myIndex + 2]['distance_AN'] = '{:0.6f}'.format(0.0)
            if 'B' in overhead['phases'] and 'C' in overhead['phases']:
                if Dbc > 0:
                    overhead[
                        myIndex + 1][myIndex + 2]['distance_BC'] = '{:0.6f}'.format(Dbc)
                else:
                    overhead[
                        myIndex + 1][myIndex + 2]['distance_BC'] = '{:0.6f}'.format(4.5)
            else:
                overhead[
                    myIndex + 1][myIndex + 2]['distance_BC'] = '{:0.6f}'.format(0.0)
            if 'B' in overhead['phases'] and 'N' in overhead['phases']:
                if Dbn > 0:
                    overhead[
                        myIndex + 1][myIndex + 2]['distance_BN'] = '{:0.6f}'.format(Dbn)
                else:
                    overhead[
                        myIndex + 1][myIndex + 2]['distance_BN'] = '{:0.6f}'.format(4.272002)
            else:
                overhead[
                    myIndex + 1][myIndex + 2]['distance_BN'] = '{:0.6f}'.format(0.0)
            if 'C' in overhead['phases'] and 'N' in overhead['phases']:
                if Dcn > 0:
                    overhead[
                        myIndex + 1][myIndex + 2]['distance_CN'] = '{:0.6f}'.format(Dcn)
                else:
                    overhead[
                        myIndex + 1][myIndex + 2]['distance_CN'] = '{:0.6f}'.format(5.0)
            else:
                overhead[
                    myIndex + 1][myIndex + 2]['distance_CN'] = '{:0.6f}'.format(0.0)
            ...
            if 'A' in overhead['phases'] and (ohLineList[19] != '0' or ohLineList[22] != '0'):
                overhead['distributed_load_A'] = float(
                    ohLineList[19]) * 1000 + float(ohLineList[22]) * 1000j
            if 'B' in overhead['phases'] and (ohLineList[20] != '0' or ohLineList[23] != '0'):
                overhead['distributed_load_B'] = float(
                    ohLineList[20]) * 1000 + float(ohLineList[23]) * 1000j
            if 'C' in overhead['phases'] and (ohLineList[21] != '0' or ohLineList[24] != '0'):
                overhead['distributed_load_C'] = float(
                    ohLineList[21]) * 1000 + float(ohLineList[24]) * 1000j
            return overhead
        ...
        def convertUgLine(ugLineList):
            ...
            if load_mix is None or load_mix[5] == 'W':  # Wye connected line
                underground['phases'] = ugLineList[
                    2] + ('N' if ugLineList[33] == '1' else '')
            else:  # Delta connected line
            ...
            underground['length'] = (
                '10' if float(ugLineList[12]) < 10 else ugLineList[12])
            ...
            if 'A' in underground['phases'] and 'B' in underground['phases']:
                ...
                underground[
                    myIndex + 1][myIndex + 2]['distance_AB'] = '{:0.1f}'.format(0.0)
            if 'A' in underground['phases'] and 'C' in underground['phases']:
                underground[
                    myIndex + 1][myIndex + 2]['distance_AC'] = '{:0.6f}'.format(Dac)
            else:
                underground[
                    myIndex + 1][myIndex + 2]['distance_AC'] = '{:0.1f}'.format(0.0)
            if 'A' in underground['phases'] and 'N' in underground['phases']:
                underground[
                    myIndex + 1][myIndex + 2]['distance_AN'] = '{:0.6f}'.format(Dan)
            else:
                underground[
                    myIndex + 1][myIndex + 2]['distance_AN'] = '{:0.1f}'.format(0.0)
            if 'B' in underground['phases'] and 'C' in underground['phases']:
                underground[
                    myIndex + 1][myIndex + 2]['distance_BC'] = '{:0.6f}'.format(Dbc)
            else:
                underground[
                    myIndex + 1][myIndex + 2]['distance_BC'] = '{:0.1f}'.format(0.0)
            if 'B' in underground['phases'] and 'N' in underground['phases']:
                underground[
                    myIndex + 1][myIndex + 2]['distance_BN'] = '{:0.6f}'.format(Dbn)
            else:
                underground[
                    myIndex + 1][myIndex + 2]['distance_BN'] = '{:0.1f}'.format(0.0)
            if 'C' in underground['phases'] and 'N' in underground['phases']:
                underground[
                    myIndex + 1][myIndex + 2]['distance_CN'] = '{:0.6f}'.format(Dcn)
            else:
                underground[
                    myIndex + 1][myIndex + 2]['distance_CN'] = '{:0.1f}'.format(0.0)
            ...
            for letter in underground['phases']:
                ...
                if hardware is None:
                    ...
                    neutral_diameter = (
                        float(hardware[9]) - float(hardware[12])) * 12
            ...
            if 'A' in underground['phases'] and (ugLineList[19] != '0' or ugLineList[22] != '0'):
                underground['distributed_load_A'] = float(ugLineList[19]) * 1000 + (
                    '+' if float(ugLineList[22]) >= 0.0 else '-') + abs(float(ugLineList[22])) * 1000j
            if 'B' in underground['phases'] and (ugLineList[20] != '0' or ugLineList[23] != '0'):
                underground['distributed_load_B'] = float(ugLineList[20]) * 1000 + (
                    '+' if float(ugLineList[23]) >= 0.0 else '-') + abs(float(ugLineList[23])) * 1000j
            if 'C' in underground['phases'] and (ugLineList[21] != '0' or ugLineList[24] != '0'):
                underground['distributed_load_C'] = float(ugLineList[21]) * 1000 + (
                    '+' if float(ugLineList[24]) >= 0.0 else '-') + abs(float(ugLineList[24])) * 1000j
            return underground
        ...
        def convertRegulator(regList):
            ...
            if reg_hardware is not None:
                ...
                if float(reg_hardware[6]) > 0.0:
                    raise_taps = str(
                        math.ceil(float(reg_hardware[4]) / float(reg_hardware[6])))
                    lower_taps = str(
                        math.ceil(float(reg_hardware[5]) / float(reg_hardware[6])))
                else:
                    raise_taps = '16'
                    lower_taps = '16'
            ...
            # MAYBEFIX: figure out whether I'll run into trouble if the
            # following integer isn't unique:
            regulator[myIndex + 1] = {}
            regulator[myIndex + 1]['name'] = regulator['name'] + '-CONFIG'
            regulator[
                myIndex + 1]['omfEmbeddedConfigObject'] = 'configuration object regulator_configuration'
            # MAYBEFIX: change these from just default values:
            ...
            if float(ldc_r_A) != 0.0 and 'A' in regulator['phases']:
                regulator[
                    myIndex + 1]['compensator_r_setting_A'] = str(float(ldc_r_A) * 120)
            if float(ldc_x_A) != 0.0 and 'A' in regulator['phases']:
                regulator[
                    myIndex + 1]['compensator_x_setting_A'] = str(float(ldc_x_A) * 120)
            if float(ldc_r_B) != 0.0 and 'B' in regulator['phases']:
                regulator[
                    myIndex + 1]['compensator_r_setting_B'] = str(float(ldc_r_B) * 120)
            if float(ldc_x_B) != 0.0 and 'B' in regulator['phases']:
                regulator[
                    myIndex + 1]['compensator_x_setting_B'] = str(float(ldc_x_B) * 120)
            if float(ldc_r_C) != 0.0 and 'C' in regulator['phases']:
                regulator[
                    myIndex + 1]['compensator_r_setting_C'] = str(float(ldc_r_C) * 120)
            if float(ldc_x_C) != 0.0 and 'C' in regulator['phases']:
                regulator[
                    myIndex + 1]['compensator_x_setting_C'] = str(float(ldc_x_C) * 120)
        ...
        def convertTransformer(transList):
            ...
            transformer[
                myIndex + 1]['omfEmbeddedConfigObject'] = 'configuration object transformer_configuration'
            transformer[
                myIndex + 1]['primary_voltage'] = str(float(transList[10]) * 1000)
            transformer[
                myIndex + 1]['secondary_voltage'] = str(float(transList[13]) * 1000)
            # Grab transformer phase hardware. NOTE: the default values were
            # averages from the test files.
            ...
            if float(no_load_loss) > 0.0:
                r_shunt = float(
                    transList[10]) * float(transList[10]) * 1000 / float(no_load_loss)
                x_shunt = r_shunt * float(x_r_ratio)
                transformer[
                    myIndex + 1]['shunt_impedance'] = str(r_shunt) + '+' + str(x_shunt) + 'j'
            # Set series impedance
            if float(percent_z) > 0.0:
                r_series = float(
                    percent_z) * 0.01 / math.sqrt(1 + (float(x_r_ratio) * float(x_r_ratio)))
                x_series = r_series * float(x_r_ratio)
                transformer[
                    myIndex + 1]['impedance'] = str(r_series) + '+' + str(x_series) + 'j'
            else:
            ...
            if 1 == len(transPhases):
                transformer[
                    myIndex + 1]['connect_type'] = 'SINGLE_PHASE_CENTER_TAPPED'
            else:
                # MAYBEFIX: support other types of windings (D-D, D-Y, etc.)
                transformer[myIndex + 1]['connect_type'] = 'WYE_WYE'
            # MAYBEFIX: change these from just default values:
            transformer[myIndex + 1]['power_rating'] = str(
                float(transList[19]) + float(transList[20]) + float(transList[21]))
            ...
            # MAYBEFIX: and change these, which were added to make the
            # transformer work on multiple phases:
            return transformer
    ...
    def fixCompConnectivity(comp):
        ...
        fromToable = ['overhead_line', 'underground_line',
                      'regulator', 'transformer', 'switch', 'fuse']
        nodable = ['node']
        parentable = ['capacitor', 'ZIPload', 'diesel_dg', 'load']

        def getByGuid(guid):
            candidates = [
                x for x in convertedComponents if 'guid' in x.keys() and x['guid'] == guid]
            if len(candidates) == 0:
                return {}
            else:
                return candidates[0]
        ...
        def getByName(name):
            candidates = [
                x for x in convertedComponents if 'name' in x.keys() and x['name'] == name]
            if len(candidates) == 0:
                return {}
            else:
                return candidates[0]
        ...
        if comp['object'] in parentable and parentType(comp) in nodable:
            ...
            if 'to' in parent.keys():
                ...
                interNode['phases'] = phaseMerge(
                    interNode['phases'], comp['phases'])
            else:
            ...
            # Here we're in an error case (like loads connected to loads), so
            # do nothing:
            return False
        return True
    ...
    # Go to a dictionary format so we have a valid glmTree. Start at 1 so we
    # have room for headers:
    glmTree = {(1 + convertedComponents.index(x))
               * subObCount: x for x in convertedComponents}

    # MAYBEFIX: REMOVE THIS DISASTER HERE AND FIGURE OUT WHY SOME LINKS ARE
    # MALFORMED
    ...
    # Strip guids:
    for key in glmTree:
        if 'guid' in glmTree[key]:
            del glmTree[key]['guid']
        if 'parentGuid' in glmTree[key]:
            del glmTree[key]['parentGuid']

    def fixLinkPhases(comp):
        def getByName(name):
            candidates = [glmTree[x] for x in glmTree if 'name' in glmTree[
                x] and glmTree[x]['name'] == name]
            if len(candidates) == 0:
                return {}
            else:
                return candidates[0]
        ...
        if comp['object'] in ['overhead_line', 'underground_line', 'regulator', 'transformer', 'switch', 'fuse']:
            ...
            if 'N' in comp['phases'] and (comp['object'] == 'overhead_line' or comp['object'] == 'underground_line'):
                ...
                for y in comp.keys():
                    if type(y) is int:
                        key = y + 5
                        # line_configuration has a neutral conductor
                        if key in comp[y].keys():
                            pass
                        else:
    ...
    def convDistLoadLines(glm_dict):
        ...
        # Grab all the keys of overhead or underground lines that have
        # distributed loads
        dl_line_keys = [x for x in glm_dict if 'distributed_load_A' in glm_dict[
            x] or 'distributed_load_B' in glm_dict[x] or 'distributed_load_C' in glm_dict[x]]
        ...
        for y in dl_line_keys:
            ...
            if node12 != None and load1 != None and load2 != None:
                ...
                if 'distributed_load_A' in glm_dict[y]:
                    load1['constant_power_A'] = str(glm_dict[y]['distributed_load_A'].real * 2 / 3) + ('+' if glm_dict[y][
                        'distributed_load_A'].imag >= 0.0 else '-') + str(abs(glm_dict[y]['distributed_load_A'].imag * 2 / 3)) + 'j'
                    load2['constant_power_A'] = str(glm_dict[y]['distributed_load_A'].real / 3) + ('+' if glm_dict[y][
                        'distributed_load_A'].imag >= 0.0 else '-') + str(abs(glm_dict[y]['distributed_load_A'].imag / 3)) + 'j'
                    del glm_dict[y]['distributed_load_A']
                if 'distributed_load_B' in glm_dict[y]:
                    load1['constant_power_B'] = str(glm_dict[y]['distributed_load_B'].real * 2 / 3) + ('+' if glm_dict[y][
                        'distributed_load_B'].imag >= 0.0 else '-') + str(abs(glm_dict[y]['distributed_load_B'].imag * 2 / 3)) + 'j'
                    load2['constant_power_B'] = str(glm_dict[y]['distributed_load_B'].real / 3) + ('+' if glm_dict[y][
                        'distributed_load_B'].imag >= 0.0 else '-') + str(abs(glm_dict[y]['distributed_load_B'].imag / 3)) + 'j'
                    del glm_dict[y]['distributed_load_B']
                if 'distributed_load_C' in glm_dict[y]:
                    load1['constant_power_C'] = str(glm_dict[y]['distributed_load_C'].real * 2 / 3) + ('+' if glm_dict[y][
                        'distributed_load_C'].imag >= 0.0 else '-') + str(abs(glm_dict[y]['distributed_load_C'].imag * 2 / 3)) + 'j'
                    load2['constant_power_C'] = str(glm_dict[y]['distributed_load_C'].real / 3) + ('+' if glm_dict[y][
                        'distributed_load_C'].imag >= 0.0 else '-') + str(abs(glm_dict[y]['distributed_load_C'].imag / 3)) + 'j'
            ...
            for a in line_segment1.keys():
                if type(line_segment1[a]) is dict:
                    line_segment1[a]['name'] = line_segment1[
                        'name'] + '-LINECONFIG'
                    for b in line_segment1[a].keys():
                        if type(line_segment1[a][b]) is dict and 'omfEmbeddedConfigObject' in line_segment1[a][b] and line_segment1[a][b]['omfEmbeddedConfigObject'] == 'spacing object line_spacing':
                            line_segment1[a][b]['name'] = line_segment1[
                                'name'] + '-LINESPACING'
            ...
            for a in line_segment2.keys():
                if type(line_segment2[a]) is dict:
                    line_segment2[a]['name'] = line_segment2[
                        'name'] + '-LINECONFIG'
                    for b in line_segment2[a].keys():
                        if type(line_segment2[a][b]) is dict and 'omfEmbeddedConfigObject' in line_segment2[a][b] and line_segment2[a][b]['omfEmbeddedConfigObject'] == 'spacing object line_spacing':
                            line_segment2[a][b]['name'] = line_segment2[
                                'name'] + '-LINESPACING'
    ...
    def fix_nominal_voltage(glm_dict, volt_dict):
        for x in glm_dict:
            if 'parent' in glm_dict[x].keys() and glm_dict[x]['parent'] in volt_dict.keys() and glm_dict[x]['name'] not in volt_dict.keys():
                glm_dict[x]['nominal_voltage'] = volt_dict[
                    glm_dict[x]['parent']]
                ...
                for y in glm_dict:
                    if 'name' in glm_dict[y] and glm_dict[y]['name'] == glm_dict[x]['to']:
                        glm_dict[y]['nominal_voltage'] = glm_dict[
                            x]['nominal_voltage']
                        volt_dict[glm_dict[y]['name']] = glm_dict[
                            y]['nominal_voltage']
        ...
        if 'object' in glmTree[x].keys() and glmTree[x]['object'] == 'regulator':
            for y in glmTree[x].keys():
                if type(glmTree[x][y]) is dict:
                    glmTree[x][y]['power_transducer_ratio'] = str(
                        float(glmTree[x]['nominal_voltage']) / 120)
    ...
    def secondarySystemFix(glm):
        def unused_key(dic, key_multiplier):
            free_key = (
                int(max(dic.keys()) / key_multiplier) + 1) * key_multiplier
            return free_key
        allLoadKeys = [x for x in glm if 'object' in glm[x] and 'parent' in glm[x] and glm[
            x]['object'] == 'load' and 'load_class' in glm[x] and glm[x]['load_class'] == 'R']
        allNamesNodesOnLoads = list(
            set([glm[key]['parent'] for key in allLoadKeys]))
        all2ndTransKeys = []
        all2ndLoadKeys = []
        all2ndNodeKeys = []
        ...
        def nameToKey(name):
            hits = [
                x for x in glm if 'name' in glm[x] and glm[x]['name'] == name]
            if len(hits) == 0:
                return []
            else:
                return hits
        for key in glm:
            if 'object' in glm[key] and glm[key]['object'] == 'transformer':
                ...
                if toName in allNamesNodesOnLoads:
                    ...
                    all2ndLoadKeys.extend([x for x in glm if 'parent' in glm[x] and 'object' in glm[
                                          x] and glm[x]['object'] == 'load' and glm[x]['parent'] == toName])
                else:
        ...
        for nodeKey in all2ndNodeKeys:
            ...
            # if the node has multiple phases we need to split it out to
            # multiple triplex_meters
            for y in phases:
                if y != 'N' and y != 'D':
        ...
        for loadKey in all2ndLoadKeys:
            ...
            for y in phases:
                if y != 'N' and y != 'D':
                    ...
                    if y == 'A':
                        glm[new_key]['power_12'] = glm[
                            loadKey]['constant_power_A']
                    elif y == 'B':
                        glm[new_key]['power_12'] = glm[
                            loadKey]['constant_power_B']
                    elif y == 'C':
                        glm[new_key]['power_12'] = glm[
                            loadKey]['constant_power_C']
                    ...
                    for z in glm[key]:
                        if type(z) is int:
                            ...
                            if 'shunt_impedance' in glm[key][z]:
                                glm[new_key][
                                    new_key + 1]['shunt_impedance'] = glm[key][z]['shunt_impedance']
                            # NOTE: default values are average of all .std data
                            # sets we've seen.
                            if y == 'A':
                                glm[new_key][
                                    new_key + 1]['power_rating'] = glm[key][z].get('powerA_rating', 250)
                                glm[new_key][
                                    new_key + 1]['powerA_rating'] = glm[key][z].get('powerA_rating', 250)
                            elif y == 'B':
                                glm[new_key][
                                    new_key + 1]['power_rating'] = glm[key][z].get('powerB_rating', 250)
                                glm[new_key][
                                    new_key + 1]['powerB_rating'] = glm[key][z].get('powerB_rating', 250)
                            elif y == 'C':
                                glm[new_key][
                                    new_key + 1]['power_rating'] = glm[key][z].get('powerC_rating', 250)
                                glm[new_key][
                                    new_key + 1]['powerC_rating'] = glm[key][z].get('powerC_rating', 250)
        # Delete original transformers, nodes and loads that the split phase
        # objects are representing
        for key in all2ndTransKeys:
            del glm[key]
    ...
    def dedupGlm(compName, glmRef):
        ...
        def dupToTup(inList):
            # Go through a list of components, and given two identicals (up to
            # name) in a row, replace the first one with (name1, name2).
        ...
        def dechain(tupleList):
            # Go backwards through a list of tuples and change e.g.
            # (1,2),(2,3),(3,4) into (1,4),(2,4),(3,4).
        ...
        compList = sorted([glmRef[x] for x in glmRef if 'object' in glmRef[x] and glmRef[x][
                          'object'] == compName], key=lambda x: {val: x[val] for val in x if val != 'name'})
        ...
        if compName == 'transformer_configuration':
            transformers = [glmRef[x] for x in glmRef if 'object' in glmRef[
                x] and glmRef[x]['object'] == 'transformer']
            for tranny in transformers:
                if tranny['configuration'] in nameDictMap.keys():
                    tranny['configuration'] = nameDictMap[
                        tranny['configuration']]
        elif compName == 'regulator_configuration':
            regulators = [glmRef[x] for x in glmRef if 'object' in glmRef[
                x] and glmRef[x]['object'] == 'regulator']
            for reggy in regulators:
                if reggy['configuration'] in nameDictMap.keys():
                    reggy['configuration'] = nameDictMap[
                        reggy['configuration']]
        elif compName == 'line_spacing':
            lineConfigs = [glmRef[x] for x in glmRef if 'object' in glmRef[
                x] and glmRef[x]['object'] == 'line_configuration']
            for config in lineConfigs:
                if config['spacing'] in nameDictMap.keys():
                    config['spacing'] = nameDictMap[config['spacing']]
    ...
    # NOTE: This last dedup has to come last, because it relies on doing
    # conductors and spacings first!
    ...
    genericHeaders = [	{"timezone": "PST+8PDT", "stoptime": "'2000-01-01 00:00:00'", "starttime": "'2000-01-01 00:00:00'", "clock": "clock"},
                       ...
                       {"solver_method": "NR", "NR_iteration_limit": "50",
                           "module": "powerflow"},
    ...
    for key in glmTree:
        ...
        if thisOb.get('object', '') == 'transformer':
            fromOb = glmTree.get(
                nameToIndex.get(thisOb.get('from', ''), 0), {})
            ...
            if toOb.get('object', '') == 'triplex_meter' and 'latitude' in fromOb and 'longitude' in fromOb:
                toOb['latitude'] = str(
                    float(fromOb['latitude']) + random.uniform(-10, 10))
                toOb['longitude'] = str(
                    float(fromOb['longitude']) + random.uniform(-10, 10))
    for key in glmTree:
        thisOb = glmTree[key]
        if thisOb.get('object', '') in ['triplex_node', 'load']:
            parentOb = glmTree.get(
                nameToIndex.get(thisOb.get('parent', ''), 0), {})
            if 'latitude' in parentOb and 'longitude' in parentOb:
                thisOb['latitude'] = str(
                    float(parentOb['latitude']) + random.uniform(-5, 5))
                thisOb['longitude'] = str(
                    float(parentOb['longitude']) + random.uniform(-5, 5))
...
def _tests(keepFiles=False):
    ...
    testFiles = [('INEC-RENOIR.std', 'INEC.seq'), ('INEC-GRAHAM.std', 'INEC.seq'),
                 ('Olin-Barre.std',
                  'Olin.seq'), ('Olin-Brown.std', 'Olin.seq'),
                 ('ABEC-Frank.std', 'ABEC.seq'), ('ABEC-COLUMBIA.std', 'ABEC.seq')]
    testAttachments = {'schedules.glm': '', 'climate.tmy2': open(
        './data/Climate/KY-LEXINGTON.tmy2', 'r').read()}
    for stdString, seqString in testFiles:
        try:
            ...
            try:
                ...
                Plot.save_fig(plt, outPrefix + stdString.replace('.std', '.png'))
            ...
            try:
                ...
                output = gridlabd.runInFilesystem(
                    outGlm, attachments=testAttachments, keepFiles=False)
