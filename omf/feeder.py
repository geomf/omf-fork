...
def adjustTime(tree, simLength, simLengthUnits, simStartDate):
    ...
    for x in tree:
        ...
        if 'clock' in leaf:
            ...
            # Also, set the file since we're already inside this data
            # structure.
...
def fullyDeEmbed(glmTree):
    ...
    '''
    logger.info('Flattening data structure, from hierarchical tree to flat list of nodes and indices')
    lenDiff = 1
    while lenDiff != 0:
        currLen = len(glmTree)
...
def attachRecorders(tree, recorderType, keyToJoin, valueToJoin):
    ...
    # HACK: the biggestKey assumption only works for a flat tree or one that
    # has a flat node for the last item...
...
def groupSwingKids(tree):
    ...
    biggestKey = sorted([int(key) for key in tree.keys()])[-1] + 1
    collector = {'interval': '1', 'object': 'collector', 'limit': '0', 'group':
                 'X', 'file': 'Y', 'property': 'sum(power_in.real),sum(power_in.imag)'}
    ...
    for key in inTree:
        ...
        if 'name' in item.keys():
            if 'parent' in item.keys():
                outGraph.add_edge(
                    item['name'], item['parent'], attr_dict={'type': 'parentChild', 'phases': 1})
                ...
                outGraph.add_edge(
                    item['from'], item['to'], attr_dict={'type': item['object'], 'phases': myPhase})
            elif item['name'] in outGraph:
                # Edge already led to node's addition, so just set the
                # attributes:
                outGraph.node[item['name']]['type'] = item['object']
            else:
                outGraph.add_node(
                    item['name'], attr_dict={'type': item['object']})
...
def latLonNxGraph(inGraph, labels=False, neatoLayout=False):
    ...
    nx.draw_networkx_nodes(inGraph, pos,
                           nodelist=pos.keys(),
                           node_color=[
                               _obToCol(inGraph.node[n]['type']) for n in inGraph],
                           linewidths=0,
                           node_size=40)
    if labels:
...
def _tokenizeGlm(inputStr, filePath=True):
    ...
    '''
    if filePath:
        logger.info('Turning a GLM file into a linked list of tokens.')
        with open(inputStr, 'r') as glmFile:
            data = glmFile.read()
    else:
        logger.info('Turning a GLM string into a linked list of tokens.')
        data = inputStr
...
def _parseTokenList(tokenList):
    ...
    logger.info('Parsing list of tokens into tree data structure.')
    ...
    while tokenList != []:
        ...
        if fullToken[-1] == ';':
            ...
            if guidStack == [] and fullToken != [';']:
                tree[guid] = {
                    'omftype': fullToken[0], 'argument': listToString(fullToken)}
                guid += 1
            ...
            if fullToken[0] == 'schedule':
                while fullToken[-1] not in ['}']:
                    fullToken.append(tokenList.pop(0))
                tree[guid] = {'object': 'schedule', 'name': fullToken[
                    1], 'cron': ' '.join(fullToken[3:-2])}
                guid += 1
        elif fullToken[-1] == '{':
            currentLeafAdd(guid, {})
            ...
            if len(fullToken) > 1:
                ...
                if len(fullToken) < 4:
                    ...
                    currentLeafAdd(
                        'omfEmbeddedConfigObject', fullToken[0] + ' ' + listToString(fullToken))
    return tree
        ...
        if type(key) is int:
            ...
            if key == 'comment':
                ...
                if len(inDict[key]) <= 62:
                    ...
                    warnings.warn("{:s} argument is longer that 64 characters. Truncating.".format(
                        key), RuntimeWarning)
                    otherKeyValues += ('\t' + key + ' ' + str(
                        inDict[key])[0:62] + '; // truncated from {:s}\n'.format(inDict[key]))
            else:
    ...
    if 'omftype' in inDict:
        ...
        if 'variable_types' in inDict.keys() and 'variable_names' in inDict.keys() and len(inDict['variable_types']) == len(inDict['variable_names']):
            for x in xrange(len(inDict['variable_types'])):
                prop += '\t' + \
                    inDict['variable_types'][x] + ' ' + \
                        inDict['variable_names'][x] + ';\n'
...
def _deEmbedOnce(glmTree):
    ...
    for x in iterTree:
        # Object name = iterTree[x]['name']
        for y in iterTree[x]:
            # SubObject name = iterTree[x][y]['name']
            if type(iterTree[x][y]) is dict and 'object' in iterTree[x][y]:
                ...
                if 'name' in glmTree[x][y]:
                    ...
                    glmTree[x][y]['name'] = glmTree[x][
                        'name'] + glmTree[x][y]['object'] + str(y)
                # check for key collision, which should technically be
                # impossible:
                if y in glmTree.keys():
                    logger.info('KEY COLLISION! - %s for x=%s', y,x)
                    z = y
