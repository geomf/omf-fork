...
def _openDatabase(database_file):
    ...
    if sys.platform == 'win32' or sys.platform == 'cygwin':
        ...
        connect_string = 'DRIVER={Microsoft Access Driver (*.mdb)};DBQ=' + str(
            database_file) + ';'
...
def _readCymeSource(networkDatabase, feederId, type):
    ...
    if (type == 1):
        CYMSOURCE = {'name': None,            # information structure for each object found in CYMSOURCE
                     ...
                     'phases': None}
        # Check to see if the network database contains models for more than
        # one database and if we chose a valid feeder_id to convert
        feeder_db = networkDatabase.execute(
            "SELECT NodeId, NetworkId, EquipmentId, DesiredVoltage FROM CYMSOURCE").fetchall()
    elif (type == 2):
        ...
        # one database and if we chose a valid feeder_id to convert
        feeder_db = networkDatabase.execute(
            "SELECT NodeId, OperatingVoltage1 FROM CYMEQUIVALENTSOURCE").fetchall()
        feeder_db_net = networkDatabase.execute(
            "SELECT NetworkId FROM CYMNETWORK").fetchall()
            ...
            if len(feeder_db) == 1:
                try:
                    for row in feeder_db:
                        ...
                        cymsource[_fixName(row.NodeId)] = copy.deepcopy(
                            CYMSOURCE)
                        cymsource[_fixName(row.NodeId)]['name'] = _fixName(
                            row.NodeId)
                        cymsource[_fixName(row.NodeId)]['nominal_voltage'] = str(
...
def _readCymeNode(networkDatabase, feederId):
    ...
    CYMNODE = {'name': None,
               'latitude': None,
               'longitude': None}
    node_db = networkDatabase.execute(
        "SELECT NodeId, X, Y FROM CYMNODE WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    if len(node_db) == 0:
        warnings.warn("No information node locations were found in CYMNODE for feeder id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
        for row in node_db:
            x_list.append(row.X)
            y_list.append(row.Y)
...
def _readCymeOverheadByPhase(networkDatabase, feederId):
    ...
    CYMOVERHEADBYPHASE = {'name': None,       # Information structure for each object found in CYMOVERHEADBYPHASE
                          'length': None,
                          'configuration': None}
    overheadbyphase_db = networkDatabase.execute(
        "SELECT DeviceNumber, PhaseConductorIdA, PhaseConductorIdB, PhaseConductorIdC, NeutralConductorId, ConductorSpacingId, Length FROM CYMOVERHEADBYPHASE WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    if len(overheadbyphase_db) == 0:
        warnings.warn("No information on phase conductors, spacing, and lengths were found in CYMOVERHEADBYPHASE for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
...
def _readCymeUndergroundLine(networkDatabase, feederId):
    ...
    cymundergroundline = {
    }                         # Stores information found in CYMUNDERGOUNDLINE in the network database
    # Stores the unique underground conductor equipment Ids
    undergroundConductors = []
    ...
    ug_line_db = networkDatabase.execute(
        "SELECT DeviceNumber, CableId, Length FROM CYMUNDERGROUNDLINE WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    if len(ug_line_db) == 0:
        warnings.warn("No underground_line objects were found in CYMUNDERGROUNDLINE for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
        for row in ug_line_db:
            row.DeviceNumber = _fixName(row.DeviceNumber)
            if row.DeviceNumber not in cymundergroundline.keys():
                cymundergroundline[row.DeviceNumber] = copy.deepcopy(
                    CYMUNDERGROUNDLINE)
                cymundergroundline[row.DeviceNumber][
                    'name'] = _fixName(row.DeviceNumber)
                cymundergroundline[row.DeviceNumber][
                    'cable_id'] = _fixName(row.CableId)
                cymundergroundline[row.DeviceNumber][
                    'length'] = float(row.Length) * m2ft
...
def _readCymeOverheadLineUnbalanced(networkDatabase, feederId):
    ...
    CYMOVERHEADLINEUNBALANCED = {'name': None,           # Information structure for each object found in CYMOVERHEADLINEUNBALANCED
                                 ...
                                 'configuration': None}
    ug_line_db = networkDatabase.execute(
        "SELECT DeviceNumber, LineId, Length FROM CYMOVERHEADLINEUNBALANCED WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    if len(ug_line_db) == 0:
        warnings.warn("No underground_line objects were found in CYMOVERHEADLINEUNBALANCED for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
        for row in ug_line_db:
            row.DeviceNumber = _fixName(row.DeviceNumber)
            if row.DeviceNumber not in cymoverheadlineunbalanced.keys():
                cymoverheadlineunbalanced[row.DeviceNumber] = copy.deepcopy(
                    CYMOVERHEADLINEUNBALANCED)
                cymoverheadlineunbalanced[row.DeviceNumber][
                    'name'] = _fixName(row.DeviceNumber)
                cymoverheadlineunbalanced[row.DeviceNumber][
                    'configuration'] = _fixName(row.LineId)
                cymoverheadlineunbalanced[row.DeviceNumber][
                    'length'] = float(row.Length) * m2ft
...
def _readCymeSection(networkDatabase, feederId):
    ...
    cymsection = {
    }                         # Stores information found in CYMSECTION in the network database
    CYMSECTION = {'name': None,           # Information structure for each object found in CYMSECTION
                  'from': None,
                  'to': None,
                  'phases': None}
    section_db = networkDatabase.execute(
        "SELECT SectionId, FromNodeId, ToNodeId, Phase FROM CYMSECTION WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    if len(section_db) == 0:
        warnings.warn("No section information was found in CYMSECTION for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
        for row in section_db:
            ...
            if row.SectionId not in cymsection.keys():
                ...
                cymsection[row.SectionId][
                    'phases'] = _convertPhase(int(row.Phase))
    return cymsection
...
def _readCymeSectionDevice(networkDatabase, feederId):
    ...
    CYMSECTIONDEVICE = {'name': None,           # Information structure for each object found in CYMSECTIONDEVICE
                        ...
                        'section_name': None,
                        'location': None}
    section_device_db = networkDatabase.execute(
        "SELECT DeviceNumber, DeviceType, SectionId, Location FROM CYMSECTIONDEVICE WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    if len(section_device_db) == 0:
        warnings.warn("No section device information was found in CYMSECTIONDEVICE for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
        for row in section_device_db:
            ...
            if row.DeviceNumber not in cymsectiondevice.keys():
                cymsectiondevice[row.DeviceNumber] = copy.deepcopy(
                    CYMSECTIONDEVICE)
                cymsectiondevice[row.DeviceNumber]['name'] = row.DeviceNumber
                cymsectiondevice[row.DeviceNumber][
                    'device_type'] = int(row.DeviceType)
                cymsectiondevice[row.DeviceNumber][
                    'section_name'] = row.SectionId
                cymsectiondevice[row.DeviceNumber][
...
def _splitLinkObjects(sectionDict, deviceDict, linkDict, overheadDict, undergroundDict):
    ...
    for link in linkDict.keys():
        ...
        if link in overheadDict.keys() or link in undergroundDict.keys():
            lineId = link
            newLinkId = linkDict[link]
            # device is at the from side of a section
            if deviceDict[newLinkId]['location'] == 1:
                ...
                sectionDict[newLinkId]['toX'] = str(
                    float(sectionDict[lineId]['fromX']) + random.uniform(-10, 10))
                sectionDict[newLinkId]['toY'] = str(
                    float(sectionDict[lineId]['fromY']) + random.uniform(-10, 10))
                ...
                sectionDict[newLinkId]['fromX'] = str(
                    float(sectionDict[lineId]['toX']) + random.uniform(-10, 10))
                sectionDict[newLinkId]['fromY'] = str(
                    float(sectionDict[lineId]['toY']) + random.uniform(-10, 10))
            ...
            for phase in ['N', 'D']:
                sectionDict[newLinkId]['phases'] = sectionDict[
                    newLinkId]['phases'].replace(phase, '')
...
def _readCymeSwitch(networkDatabase, feederId):
    # Stores information found in CYMSWITCH in the network database
    cymswitch = {}
    CYMSWITCH = {'name': None,            # Information structure for each object found in CYMSWITCH
                 'equipment_name': None,
                 'status': None}
    switch_db = networkDatabase.execute(
        "SELECT DeviceNumber, EquipmentId, ClosedPhase FROM CYMSWITCH WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    if len(switch_db) == 0:
        warnings.warn("No switch objects were found in CYMSWITCH for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
...
def _readCymeSectionalizer(networkDatabase, feederId):
    ...
    CYMSECTIONALIZER = {'name': None,             # Information structure for each object found in CYMSECTIONALIZER
                        'status': None}
    sectionalizer_db = networkDatabase.execute(
        "SELECT DeviceNumber, NormalStatus FROM CYMSECTIONALIZER WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    if len(sectionalizer_db) == 0:
        warnings.warn("No sectionalizer objects were found in CYMSECTIONALIZER for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
        for row in sectionalizer_db:
            row.DeviceNumber = _fixName(row.DeviceNumber)
            if row.DeviceNumber not in cymsectionalizer.keys():
                cymsectionalizer[row.DeviceNumber] = copy.deepcopy(
                    CYMSECTIONALIZER)
...
def _readCymeFuse(networkDatabase, feederId):
    # Stores information found in CYMFUSE in the network database
    cymfuse = {}
    CYMFUSE = {'name': None,             # Information structure for each object found in CYMFUSE
               'status': None,
               'equipment_id': None}
    fuse_db = networkDatabase.execute(
        "SELECT DeviceNumber, EquipmentId, NormalStatus FROM CYMFUSE WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    if len(fuse_db) == 0:
        warnings.warn("No fuse objects were found in CYMFUSE for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
        for row in fuse_db:
            row.DeviceNumber = _fixName(row.DeviceNumber)
...
def _readCymeRecloser(networkDatabase, feederId):
    ...
    CYMRECLOSER = {'name': None,
                   'status': None}
    recloser_db = networkDatabase.execute(
        "SELECT DeviceNumber, NormalStatus FROM CYMRECLOSER WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    if len(recloser_db) == 0:
        warnings.warn("No recloser objects were found in CYMRECLOSER for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
...
def _readCymeRegulator(networkDatabase, feederId):
    ...
    CYMREGULATOR = {'name': None,             # Information structure for each object found in CYMREGULATOR
                    ...
                    'tap_pos_B': None,
                    'tap_pos_C': None}
    regulator_db = networkDatabase.execute(
        "SELECT DeviceNumber, EquipmentId, BandWidth, BoostPercent, TapPositionA, TapPositionB, TapPositionC FROM CYMREGULATOR WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    if len(regulator_db) == 0:
        warnings.warn("No regulator objects were found in CYMREGULATOR for feeder_id: {:s}".format(
            feederId), RuntimeWarning)
    else:
        for row in regulator_db:
            ...
            if row.DeviceNumber not in cymregulator.keys():
                ...
                cymregulator[row.DeviceNumber][
                    'equipment_name'] = row.EquipmentId
                cymregulator[row.DeviceNumber][
                    'band_width'] = float(row.BandWidth) / 120.0
                cymregulator[row.DeviceNumber][
                    'regulation'] = float(row.BoostPercent) / 100.0
...
def _readCymeShuntCapacitor(networkDatabase, feederId, type):
    ...
    if (type == 1):
        ...
        shuntcapacitor_db = networkDatabase.execute(
            "SELECT DeviceNumber, EquipmentId, Status, Phase, KVARA, KVARB, KVARC, KVLN, CapacitorControlType, OnValue, OffValue, KVARABC FROM CYMSHUNTCAPACITOR WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    elif (type == 2):
        CYMSHUNTCAPACITOR = {'name': None,             # Information structure for each object found in CYMSHUNTCAPACITOR
                             'equipment_name': None,
                             'status': None,
        ...
        shuntcapacitor_db = networkDatabase.execute(
            "SELECT DeviceNumber, NetworkId, EquipmentId, Status, KVARA, KVARB, KVARC, KVLN, CapacitorControlType, OnValueA, OffValueA FROM CYMSHUNTCAPACITOR WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    if len(shuntcapacitor_db) == 0:
        warnings.warn("No capacitor objects were found in CYMSHUNTCAPACITOR for feeder_id: {:s}".format(
            feederId), RuntimeWarning)
    else:
        if (feederId == '650'):
            for row in shuntcapacitor_db:
                ...
                if row.DeviceNumber not in cymshuntcapacitor.keys():
                    cymshuntcapacitor[row.DeviceNumber] = copy.deepcopy(
                        CYMSHUNTCAPACITOR)
                    cymshuntcapacitor[row.DeviceNumber][
                    ...
                    if float(row.KVARA) == 0.0 and float(row.KVARA) == 0.0 and float(row.KVARA) == 0.0 and float(row.KVARABC) > 0.0:
                        cymshuntcapacitor[row.DeviceNumber][
                            'capacitor_A'] = float(row.KVARABC) * 1000 / 3
                        cymshuntcapacitor[row.DeviceNumber][
                            'capacitor_B'] = float(row.KVARABC) * 1000 / 3
                        cymshuntcapacitor[row.DeviceNumber][
                            'capacitor_C'] = float(row.KVARABC) * 1000 / 3
                    else:
                        if float(row.KVARA) > 0.0:
                            cymshuntcapacitor[row.DeviceNumber][
                                'capacitor_A'] = float(row.KVARA) * 1000
                        if float(row.KVARB) > 0.0:
                            cymshuntcapacitor[row.DeviceNumber][
                                'capacitor_B'] = float(row.KVARB) * 1000
                        if float(row.KVARC) > 0.0:
                            cymshuntcapacitor[row.DeviceNumber][
                                'capacitor_C'] = float(row.KVARC) * 1000
                    if float(row.KVLN) > 0.0:
                        cymshuntcapacitor[row.DeviceNumber][
                            'kV_line_neutral'] = float(row.KVLN) * 1000
                    if int(row.CapacitorControlType) == 2:
                        cymshuntcapacitor[row.DeviceNumber]['control'] = 'VAR'
                        cymshuntcapacitor[row.DeviceNumber][
                            'VAr_set_high'] = float(row.OnValue) * 1000
                        cymshuntcapacitor[row.DeviceNumber][
                            'VAr_set_low'] = float(row.OffValue) * 1000
                        cymshuntcapacitor[row.DeviceNumber][
                            'pt_phase'] = _convertPhase(int(row.Phase))
                    elif int(row.CapacitorControlType) == 3:
                        cymshuntcapacitor[row.DeviceNumber][
                            'control'] = 'CURRENT'
                        cymshuntcapacitor[row.DeviceNumber][
                            'current_set_high'] = row.OnValue
                        cymshuntcapacitor[row.DeviceNumber][
                            'current_set_low'] = row.OffValue
                        cymshuntcapacitor[row.DeviceNumber][
                            'pt_phase'] = _convertPhase(int(row.Phase))
                    elif int(row.CapacitorControlType) == 7:
                        cymshuntcapacitor[row.DeviceNumber]['control'] = 'VOLT'
                        cymshuntcapacitor[row.DeviceNumber][
                            'voltage_set_high'] = row.OnValue
                        cymshuntcapacitor[row.DeviceNumber][
                            'voltage_set_low'] = row.OffValue
                        cymshuntcapacitor[row.DeviceNumber][
                            'pt_phase'] = _convertPhase(int(row.Phase))
                    else:
                        cymshuntcapacitor[row.DeviceNumber][
                        ...
                        cymshuntcapacitor[row.DeviceNumber][
                            'pt_phase'] = _convertPhase(int(row.Phase))
                        cymshuntcapacitor[row.DeviceNumber][
                            'voltage_set_high'] = float(row.KVLN) * 1000
                        cymshuntcapacitor[row.DeviceNumber][
                            'voltage_set_low'] = float(row.KVLN) * 1000
        elif (feederId == 'BN160'):
            for row in shuntcapacitor_db:
                row.DeviceNumber = _fixName(row.DeviceNumber)
                if row.EquipmentId is None:
                ...
                if row.DeviceNumber not in cymshuntcapacitor.keys():
                    ...
                    if int(row.CapacitorControlType) == 2:
                        cymshuntcapacitor[row.DeviceNumber]['control'] = 'VAR'
                        cymshuntcapacitor[row.DeviceNumber][
                            'VAr_set_high'] = float(row.OnValueA) * 1000
                        cymshuntcapacitor[row.DeviceNumber][
                            'VAr_set_low'] = float(row.OffValueA) * 1000
                        cymshuntcapacitor[row.DeviceNumber][
                            'pt_phase'] = "ABCN"
                    elif int(row.CapacitorControlType) == 3:
                        cymshuntcapacitor[row.DeviceNumber][
                            'control'] = 'CURRENT'
                        cymshuntcapacitor[row.DeviceNumber][
                            'current_set_high'] = row.OnValueA
                        cymshuntcapacitor[row.DeviceNumber][
                            'current_set_low'] = row.OffValueA
                        cymshuntcapacitor[row.DeviceNumber][
                            'pt_phase'] = "ABCN"
                    elif int(row.CapacitorControlType) == 7:
                        cymshuntcapacitor[row.DeviceNumber]['control'] = 'VOLT'
                        cymshuntcapacitor[row.DeviceNumber][
                            'voltage_set_high'] = row.OnValueA
                        cymshuntcapacitor[row.DeviceNumber][
                            'voltage_set_low'] = row.OffValueA
                        cymshuntcapacitor[row.DeviceNumber][
                            'pt_phase'] = "ABCN"
                    else:
                        cymshuntcapacitor[row.DeviceNumber][
...
def _determineLoad(l_type, l_v1, l_v2, conKVA):
    ...
    if l_type == 0:  # information was stored as kW & kVAR
        ...
        l_real = l_v1 * 1000.0
        if l_v2 != 0.0:
            l_imag = l_real / (abs(l_v2) / 100.0) * \
                math.sqrt(1 - (abs(l_v2) / 100.0)**2)
...
def _readCymeCustomerLoad(networkDatabase, feederId):
    ...
    load_real = 0
    load_imag = 0
    customerload_db = networkDatabase.execute(
        "SELECT DeviceNumber, DeviceType, ConsumerClassId, Phase, LoadValueType, Phase, LoadValue1, LoadValue2, ConnectedKVA FROM CYMCUSTOMERLOAD WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    if len(customerload_db) == 0:
        warnings.warn("No load objects were found in CYMCUSTOMERLOAD for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
        for row in customerload_db:
            ...
            if row.DeviceNumber not in cymcustomerload.keys():
                # check for 0 load
                [load_real, load_imag] = _determineLoad(int(row.LoadValueType), float(
                    row.LoadValue1), float(row.LoadValue2), row.ConnectedKVA)
                cymcustomerload[row.DeviceNumber] = copy.deepcopy(
                    CYMCUSTOMERLOAD)
                cymcustomerload[row.DeviceNumber]['name'] = row.DeviceNumber
                cymcustomerload[row.DeviceNumber][
                    'phases'] = _cleanPhases(_convertPhase(int(row.Phase)))
                ...
                if convert_class is not None:
                    cymcustomerload[row.DeviceNumber][
                        'load_class'] = convert_class

                if int(row.Phase) == 1:
                    cymcustomerload[row.DeviceNumber]['load_realA'] = load_real
                    cymcustomerload[row.DeviceNumber]['load_imagA'] = load_imag
                    cymcustomerload[row.DeviceNumber]['constant_power_A'] = _setConstantPower(
                        float(row.LoadValue2), load_real, load_imag)
                    ...
                    cymcustomerload[row.DeviceNumber]['constant_power_B'] = _setConstantPower(
                        float(row.LoadValue2), load_real, load_imag)
                    ...
                    cymcustomerload[row.DeviceNumber]['constant_power_C'] = _setConstantPower(
                        float(row.LoadValue2), load_real, load_imag)
                elif int(row.Phase) == 7:
                    cymcustomerload[row.DeviceNumber][
                        'load_realA'] = load_real / 3.0
                    cymcustomerload[row.DeviceNumber][
                        'load_imagA'] = load_imag / 3.0
                    cymcustomerload[row.DeviceNumber]['constant_power_A'] = _setConstantPower(
                        float(row.LoadValue2), load_real / 3.0, load_imag / 3.0)
                    cymcustomerload[row.DeviceNumber][
                        'load_realB'] = load_real / 3.0
                    cymcustomerload[row.DeviceNumber][
                        'load_imagB'] = load_imag / 3.0
                    cymcustomerload[row.DeviceNumber]['constant_power_B'] = _setConstantPower(
                        float(row.LoadValue2), load_real / 3.0, load_imag / 3.0)
                    cymcustomerload[row.DeviceNumber][
                        'load_realC'] = load_real / 3.0
                    cymcustomerload[row.DeviceNumber][
                        'load_imagC'] = load_imag / 3.0
                    cymcustomerload[row.DeviceNumber]['constant_power_C'] = _setConstantPower(
                        float(row.LoadValue2), load_real / 3.0, load_imag / 3.0)
            else:
                [load_real, load_imag] = _determineLoad(int(row.LoadValueType), float(
                    row.LoadValue1), float(row.LoadValue2), float(row.ConnectedKVA))
                ph = cymcustomerload[row.DeviceNumber][
                    'phases'] + _convertPhase(int(row.Phase))
                cymcustomerload[row.DeviceNumber]['phases'] = _cleanPhases(ph)
                if int(row.Phase) == 1:
                    cymcustomerload[row.DeviceNumber][
                        'load_realA'] += load_real
                    cymcustomerload[row.DeviceNumber][
                        'load_imagA'] += load_imag
                    cymcustomerload[row.DeviceNumber]['constant_power_A'] = _setConstantPower(float(
                        row.LoadValue2), cymcustomerload[row.DeviceNumber]['load_realA'], cymcustomerload[row.DeviceNumber]['load_imagA'])
                elif int(row.Phase) == 2:
                    cymcustomerload[row.DeviceNumber][
                        'load_realB'] += load_real
                    cymcustomerload[row.DeviceNumber][
                        'load_imagB'] += load_imag
                    cymcustomerload[row.DeviceNumber]['constant_power_B'] = _setConstantPower(float(
                        row.LoadValue2), cymcustomerload[row.DeviceNumber]['load_realB'], cymcustomerload[row.DeviceNumber]['load_imagB'])
                elif int(row.Phase) == 3:
                    cymcustomerload[row.DeviceNumber][
                        'load_realC'] += load_real
                    cymcustomerload[row.DeviceNumber][
                        'load_imagC'] += load_imag
                    cymcustomerload[row.DeviceNumber]['constant_power_C'] = _setConstantPower(float(
                        row.LoadValue2), cymcustomerload[row.DeviceNumber]['load_realC'], cymcustomerload[row.DeviceNumber]['load_imagC'])
                elif int(row.Phase) == 7:
                    cymcustomerload[row.DeviceNumber][
                        'load_realA'] += load_real / 3.0
                    cymcustomerload[row.DeviceNumber][
...
def _readCymeThreeWindingTransformer(networkDatabase, feederId):
    ...
    CYMTHREEWXFMR = {'name': None,             # Information structure for each object found in CYMREGULATOR
                     'equipment_name': None}
    threewxfmr_db = networkDatabase.execute(
        "SELECT DeviceNumber, EquipmentId FROM CYMTHREEWINDINGTRANSFORMER WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    if len(threewxfmr_db) == 0:
        warnings.warn(
            "No three-winding transformer objects were found in CYMTHREEWINDINGTRANSFORMER for feeder_id: {:s}".format(feederId), RuntimeWarning)
    else:
        for row in threewxfmr_db:
            ...
            if row.DeviceNumber not in cymthreewxfmr.keys():
                ...
                cymthreewxfmr[row.DeviceNumber][
                    'equipment_name'] = row.EquipmentId
...
def _readCymeTransformer(networkDatabase, feederId):
    ...
    xfmrDb = networkDatabase.execute(
        "SELECT DeviceNumber, EquipmentId FROM CYMTRANSFORMER WHERE NetworkId = '{:s}'".format(feederId)).fetchall()
    if len(xfmrDb) == 0:
        warnings.warn("No transformer objects were found in CYMTRANSFORMER for feeder id: {:s}".format(
            feederId), RuntimeWarning)
    else:
        for row in xfmrDb:
            row.DeviceNumber = _fixName(row.DeviceNumber)
...
def _readEqConductor(equipmentDatabase, feederId):
    ...
    CYMEQCONDUCTOR = {'name': None,             # Information structure for each object found in CYMEQCONDUCTOR
                      ...
                      'resistance': None}
    cymeqconductor_db = equipmentDatabase.execute(
        "SELECT EquipmentId, FirstRating, GMR, R50 FROM CYMEQCONDUCTOR").fetchall()
    if len(cymeqconductor_db) == 0:
        warnings.warn("No conductor objects were found in CYMEQCONDUCTOR for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
        for row in cymeqconductor_db:
            ...
            if row.EquipmentId not in cymeqconductor.keys():
                ...
                cymeqconductor[row.EquipmentId][
                    'rating.summer_continuous'] = row.FirstRating
                # GMR is stored in cm. Must convert to ft.
                cymeqconductor[row.EquipmentId][
                    'geometric_mean_radius'] = float(row.GMR) * m2ft / 100
                cymeqconductor[row.EquipmentId]['resistance'] = float(
                    row.R50) * 5280 / (m2ft * 1000)  # R50 is stored in Ohm/km. Must convert to Ohm/mile
...
def _readEqOverheadLineUnbalanced(networkDatabase, feederId):
    ...
    CYMEQOVERHEADLINEUNBALANCED = {'object': 'line_configuration',
                                   ...
                                   'z33': None}
    ug_line_db = networkDatabase.execute(
        "SELECT EquipmentId, SelfResistanceA, SelfResistanceB, SelfResistanceC, SelfReactanceA, SelfReactanceB, SelfReactanceC, MutualResistanceAB, MutualResistanceBC, MutualResistanceCA, MutualReactanceAB, MutualReactanceBC, MutualReactanceCA FROM CYMEQOVERHEADLINEUNBALANCED WHERE EquipmentId = '{:s}'".format("LINE606")).fetchall()
    if len(ug_line_db) == 0:
        warnings.warn("No underground_line configuration objects were found in CYMEQOVERHEADLINEUNBALANCED for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
        for row in ug_line_db:
            row.EquipmentId = _fixName(row.EquipmentId)
            if row.EquipmentId not in cymeqoverheadlineunbalanced.keys():
                cymeqoverheadlineunbalanced[row.EquipmentId] = copy.deepcopy(
                    CYMEQOVERHEADLINEUNBALANCED)
                cymeqoverheadlineunbalanced[row.EquipmentId][
                    'name'] = _fixName(row.EquipmentId)
                cymeqoverheadlineunbalanced[row.EquipmentId]['z11'] = '{:0.6f}{:+0.6}j'.format(float(row.SelfResistanceA) * 5280 / (
                    m2ft * 1000), float(row.SelfReactanceA) * 5280 / (m2ft * 1000))  # Ohm/km. Must convert to Ohm/mile
                cymeqoverheadlineunbalanced[row.EquipmentId]['z22'] = '{:0.6f}{:+0.6}j'.format(float(
                    row.SelfResistanceB) * 5280 / (m2ft * 1000), float(row.SelfReactanceB) * 5280 / (m2ft * 1000))
                cymeqoverheadlineunbalanced[row.EquipmentId]['z33'] = '{:0.6f}{:+0.6}j'.format(float(
                    row.SelfResistanceC) * 5280 / (m2ft * 1000), float(row.SelfReactanceC) * 5280 / (m2ft * 1000))
                cymeqoverheadlineunbalanced[row.EquipmentId]['z12'] = '{:0.6f}{:+0.6}j'.format(float(
                    row.MutualResistanceAB) * 5280 / (m2ft * 1000), float(row.MutualReactanceAB) * 5280 / (m2ft * 1000))
                cymeqoverheadlineunbalanced[row.EquipmentId]['z21'] = '{:0.6f}{:+0.6}j'.format(float(
                    row.MutualResistanceAB) * 5280 / (m2ft * 1000), float(row.MutualReactanceAB) * 5280 / (m2ft * 1000))
                cymeqoverheadlineunbalanced[row.EquipmentId]['z23'] = '{:0.6f}{:+0.6}j'.format(float(
                    row.MutualResistanceBC) * 5280 / (m2ft * 1000), float(row.MutualReactanceBC) * 5280 / (m2ft * 1000))
                cymeqoverheadlineunbalanced[row.EquipmentId]['z32'] = '{:0.6f}{:+0.6}j'.format(float(
                    row.MutualResistanceBC) * 5280 / (m2ft * 1000), float(row.MutualReactanceBC) * 5280 / (m2ft * 1000))
                cymeqoverheadlineunbalanced[row.EquipmentId]['z13'] = '{:0.6f}{:+0.6}j'.format(float(
                    row.MutualResistanceCA) * 5280 / (m2ft * 1000), float(row.MutualReactanceCA) * 5280 / (m2ft * 1000))
                cymeqoverheadlineunbalanced[row.EquipmentId]['z31'] = '{:0.6f}{:+0.6}j'.format(float(
                    row.MutualResistanceCA) * 5280 / (m2ft * 1000), float(row.MutualReactanceCA) * 5280 / (m2ft * 1000))
    return cymeqoverheadlineunbalanced


def _readEqGeometricalArrangement(equipmentDatabase, feederId):
    # Stores information found in CYMEQGEOMETRICALARRANGEMENT in the equipment
    # database
    ...
    CYMEQGEOMETRICALARRANGEMENT = {'name': None,             # Information structure for each object found in CYMEQGEOMETRICALARRANGEMENT
                                   ...
                                   'distance_BN': None,
                                   'distance_CN': None}
    cymeqgeometricalarrangement_db = equipmentDatabase.execute(
        "SELECT EquipmentId, ConductorA_Horizontal, ConductorA_Vertical, ConductorB_Horizontal, ConductorB_Vertical, ConductorC_Horizontal, ConductorC_Vertical, NeutralConductor_Horizontal, NeutralConductor_Vertical FROM CYMEQGEOMETRICALARRANGEMENT").fetchall()
    if len(cymeqgeometricalarrangement_db) == 0:
        warnings.warn("No geometric spacing information was found in CYMEQGEOMETRICALARRANGEMENT for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
        for row in cymeqgeometricalarrangement_db:
            row.EquipmentId = _fixName(row.EquipmentId)
            if row.EquipmentId not in cymeqgeometricalarrangement.keys():
                cymeqgeometricalarrangement[row.EquipmentId] = copy.deepcopy(
                    CYMEQGEOMETRICALARRANGEMENT)
                cymeqgeometricalarrangement[row.EquipmentId][
                    'name'] = row.EquipmentId
                cymeqgeometricalarrangement[row.EquipmentId]['distance_AB'] = math.sqrt((float(row.ConductorA_Horizontal) - float(row.ConductorB_Horizontal))**2 + (
                    float(row.ConductorA_Vertical) - float(row.ConductorB_Vertical))**2) * m2ft  # information is stored in meters. must convert to feet.
                cymeqgeometricalarrangement[row.EquipmentId]['distance_AC'] = math.sqrt((float(row.ConductorA_Horizontal) - float(row.ConductorC_Horizontal))**2 + (
                    float(row.ConductorA_Vertical) - float(row.ConductorC_Vertical))**2) * m2ft  # information is stored in meters. must convert to feet.
                cymeqgeometricalarrangement[row.EquipmentId]['distance_AN'] = math.sqrt((float(row.ConductorA_Horizontal) - float(row.NeutralConductor_Horizontal))**2 + (
                    float(row.ConductorA_Vertical) - float(row.NeutralConductor_Vertical))**2) * m2ft  # information is stored in meters. must convert to feet.
                cymeqgeometricalarrangement[row.EquipmentId]['distance_BC'] = math.sqrt((float(row.ConductorC_Horizontal) - float(row.ConductorB_Horizontal))**2 + (
                    float(row.ConductorC_Vertical) - float(row.ConductorB_Vertical))**2) * m2ft  # information is stored in meters. must convert to feet.
                cymeqgeometricalarrangement[row.EquipmentId]['distance_BN'] = math.sqrt((float(row.NeutralConductor_Horizontal) - float(row.ConductorB_Horizontal))**2 + (
                    float(row.NeutralConductor_Vertical) - float(row.ConductorB_Vertical))**2) * m2ft  # information is stored in meters. must convert to feet.
                cymeqgeometricalarrangement[row.EquipmentId]['distance_CN'] = math.sqrt((float(row.ConductorC_Horizontal) - float(row.NeutralConductor_Horizontal))**2 + (
                    float(row.ConductorC_Vertical) - float(row.NeutralConductor_Vertical))**2) * m2ft  # information is stored in meters. must convert to feet.
...
def _readUgConfiguration(conductorDataCsv, feederId):
    ...
    if conductorDataCsv != None:
        # skip the first row as it is header information
        underground_cable_array = _csvToArray(conductorDataCsv)[1:]
        for row in underground_cable_array:
            if _fixName(row[0]) not in cymcsvundergroundcable.keys():
                cymcsvundergroundcable[_fixName(row[0])] = copy.deepcopy(
                    CYMCSVUNDERGROUNDCABLE)
                cymcsvundergroundcable[
                    _fixName(row[0])]['name'] = _fixName(row[0])
                cymcsvundergroundcable[
                    _fixName(row[0])]['conductor_resistance'] = row[2]
                cymcsvundergroundcable[
                    _fixName(row[0])]['conductor_gmr'] = row[11]
                cymcsvundergroundcable[
                    _fixName(row[0])]['rating.summer_continuous'] = row[16]
                cymcsvundergroundcable[
                    _fixName(row[0])]['conductor_diameter'] = row[17]
                cymcsvundergroundcable[
                    _fixName(row[0])]['neutral_resistance'] = row[19]
                cymcsvundergroundcable[
                    _fixName(row[0])]['neutral_gmr'] = row[21]
                cymcsvundergroundcable[
                    _fixName(row[0])]['neutral_diameter'] = row[18]
                cymcsvundergroundcable[
                    _fixName(row[0])]['neutral_strands'] = row[20]
                cymcsvundergroundcable[
                    _fixName(row[0])]['outer_diameter'] = row[19]
                cymcsvundergroundcable[
                    _fixName(row[0])]['distance_AB'] = row[24]
                cymcsvundergroundcable[
                    _fixName(row[0])]['distance_AC'] = row[26]
                cymcsvundergroundcable[
                    _fixName(row[0])]['distance_AN'] = row[27]
                cymcsvundergroundcable[
                    _fixName(row[0])]['distance_BC'] = row[25]
                cymcsvundergroundcable[
                    _fixName(row[0])]['distance_BN'] = row[28]
                cymcsvundergroundcable[
                    _fixName(row[0])]['distance_CN'] = row[29]
    else:
        warnings.warn("No conductor data spreadsheet is provided for feeder_id {:s}.".format(
            feederId), RuntimeWarning)
...
def _readEqAvgGeometricalArrangement(equipmentDatabase, feederId):
    ...
    CYMEQGEOMETRICALARRANGEMENT = {'name': None,
                                   ...
                                   'distance_BN': None,
                                   'distance_CN': None}
    cymeqaveragegeoarrangement_db = equipmentDatabase.execute(
        "SELECT EquipmentId, GMDPhaseToPhase, GMDPhaseToNeutral FROM CYMEQAVERAGEGEOARRANGEMENT").fetchall()
    if len(cymeqaveragegeoarrangement_db) == 0:
        warnings.warn("No average spacing information was found in CYMEQAVERAGEGEOARRANGEMENT for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
        for row in cymeqaveragegeoarrangement_db:
            ...
            if row.EquipmentId not in cymeqgeometricalarrangement.keys():
                ...
                cymeqgeometricalarrangement[row.EquipmentId][
                    'distance_AB'] = float(row.GMDPhaseToPhase) * m2ft
                cymeqgeometricalarrangement[row.EquipmentId][
                    'distance_AC'] = float(row.GMDPhaseToPhase) * m2ft
                cymeqgeometricalarrangement[row.EquipmentId][
                    'distance_AN'] = float(row.GMDPhaseToNeutral) * m2ft
                cymeqgeometricalarrangement[row.EquipmentId][
                    'distance_BC'] = float(row.GMDPhaseToPhase) * m2ft
                cymeqgeometricalarrangement[row.EquipmentId][
                    'distance_BN'] = float(row.GMDPhaseToNeutral) * m2ft
                cymeqgeometricalarrangement[row.EquipmentId][
                    'distance_CN'] = float(row.GMDPhaseToNeutral) * m2ft
    return cymeqgeometricalarrangement
...
def _readEqRegulator(equipmentDatabase, feederId):
    ...
    CYMEQREGULATOR = {'name': None,             # Information structure for each object found in CYMEQREGULATOR
                      'raise_taps': None,
                      'lower_taps': None}
    cymeqregulator_db = equipmentDatabase.execute(
        "SELECT EquipmentId, NumberOfTaps FROM CYMEQREGULATOR").fetchall()
    if len(cymeqregulator_db) == 0:
        warnings.warn("No regulator equipment was found in CYMEQREGULATOR for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
        for row in cymeqregulator_db:
            ...
            if row.EquipmentId not in cymeqregulator.keys():
                ...
                cymeqregulator[row.EquipmentId]['raise_taps'] = str(
                    int(float(row.NumberOfTaps) * 0.5))
                cymeqregulator[row.EquipmentId]['lower_taps'] = str(
                    int(float(row.NumberOfTaps) * 0.5))
...
def _readEqThreeWAutoXfmr(equipmentDatabase, feederId):
    ...
    cymeqthreewautoxfmr_db = equipmentDatabase.execute(
        "SELECT EquipmentId, PrimaryRatedCapacity, PrimaryVoltage, SecondaryVoltage, PrimarySecondaryZ1, PrimarySecondaryZ0, PrimarySecondaryXR1Ratio, PrimarySecondaryXR0Ratio  FROM CYMEQTHREEWINDAUTOTRANSFORMER").fetchall()
    if len(cymeqthreewautoxfmr_db) == 0:
        warnings.warn("No average spacing information was found in CYMEQTHREEWINDAUTOTRANSFORMER for feeder_id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
        for row in cymeqthreewautoxfmr_db:
            row.EquipmentId = _fixName(row.EquipmentId)
            if row.EquipmentId not in cymeqthreewautoxfmr.keys():
                cymeqthreewautoxfmr[row.EquipmentId] = copy.deepcopy(
                    CYMEQTHREEWAUTOXFMR)
                cymeqthreewautoxfmr[row.EquipmentId]['name'] = row.EquipmentId
                cymeqthreewautoxfmr[row.EquipmentId][
                    'PrimaryRatedCapacity'] = float(row.PrimaryRatedCapacity)
                cymeqthreewautoxfmr[row.EquipmentId]['PrimaryVoltage'] = float(
                    row.PrimaryVoltage) * 1000.0 / math.sqrt(3.0)
                cymeqthreewautoxfmr[row.EquipmentId]['SecondaryVoltage'] = float(
                    row.SecondaryVoltage) * 1000.0 / math.sqrt(3.0)
                if cymeqthreewautoxfmr[row.EquipmentId]['PrimaryVoltage'] == cymeqthreewautoxfmr[row.EquipmentId]['SecondaryVoltage']:
                    cymeqthreewautoxfmr[row.EquipmentId][
                        'SecondaryVoltage'] += 0.1
                z1mag = float(row.PrimarySecondaryZ1) / 100.0
                r = z1mag / \
                    math.sqrt(1 + (float(row.PrimarySecondaryXR1Ratio))**2)
                if r == 0.0:
                    r = 0.000333
                    x = 0.00222
                else:
                ...
                cymeqthreewautoxfmr[row.EquipmentId][
                    'impedance'] = '{:0.6f}{:+0.6f}j'.format(r, x)
...
def _readEqAutoXfmr(equipmentDatabase, feederId):
    ...
    CYMEQAUTOXFMR = {'name': None,
                     ...
                     'SecondaryVoltage': None,
                     'impedance': None}
    cymeqautoxfmr_db = equipmentDatabase.execute(
        "SELECT EquipmentId, NominalRatingKVA, PrimaryVoltageKVLL, SecondaryVoltageKVLL, PosSeqImpedancePercent, XRRatio FROM CYMEQAUTOTRANSFORMER").fetchall()
    if len(cymeqautoxfmr_db) == 0:
        warnings.warn("No average autotransformer equipment information was found in CYMEQAUTOTRANSFORMER for feeder id: {:s}.".format(
            feederId), RuntimeWarning)
    else:
        for row in cymeqautoxfmr_db:
            ...
            if row.EquipmentId not in cymeqautoxfmr.keys():
                ...
                cymeqautoxfmr[row.EquipmentId][
                    'PrimaryRatedCapacity'] = float(row.NominalRatingKVA)
                cymeqautoxfmr[row.EquipmentId]['PrimaryVoltage'] = float(
                    row.PrimaryVoltageKVLL) * 1000.0 / math.sqrt(3.0)
                cymeqautoxfmr[row.EquipmentId]['SecondaryVoltage'] = float(
                    row.SecondaryVoltageKVLL) * 1000.0 / math.sqrt(3.0)
...
def _find_SPCT_rating(load_str):
    ...
    spct_rating = [5, 10, 15, 25, 30, 37.5, 50, 75, 87.5, 100, 112.5, 125, 137.5, 150, 162.5, 175, 187.5, 200,
                   225, 250, 262.5, 300, 337.5, 400, 412.5, 450, 500, 750, 1000, 1250, 1500, 2000, 2500, 3000, 4000, 5000]
...
def convertCymeModel(network_db, equipment_db, test=False, type=1, feeder_id=None):
    logger.info('Converting Cyme format to Gridlab. networkMdb: %s; equipementMdb: %s; test: %s; type: %s, feederId %s',
                network_db, equipment_db, test, type, feeder_id)
    ...
    load_sections = {}
    threewxfmr_sections = {}
    logger.info('Open the network database file')
    net_db = _openDatabase(network_db)
    logger.info('-1-CYME CYMSOURCE ******************************************************')
    cymsource, feeder_id, swingBus = _readCymeSource(net_db, feeder_id, type)
    logger.info('-2-CYME CYMNODE ********************************************************')
    cymnode, x_scale, y_scale = _readCymeNode(net_db, feeder_id)
    logger.info('-3-CYME OVERHEADBYPHASE ************************************************')
    OH_conductors, cymoverheadbyphase, ohConfigurations, uniqueOhSpacing = _readCymeOverheadByPhase(
        net_db, feeder_id)
    logger.info('-4-CYME UNDERGROUNDLINE ************************************************')
    UG_conductors, cymundergroundline = _readCymeUndergroundLine(
        net_db, feeder_id)
    logger.info('-5-CYME CYMOVERHEADLINEBALANCED ****************************************')
    cymUnbalancedOverheadLine, UOLConfigNames = _readCymeOverheadLineUnbalanced(
        net_db, feeder_id)
    logger.info('-5-CYME CYMSWITCH*******************************************************')
    cymswitch = _readCymeSwitch(net_db, feeder_id)
    logger.info('-6-CYME CYMSECTIONALIZER************************************************')
    cymsectionalizer = _readCymeSectionalizer(net_db, feeder_id)
    logger.info('-7-CYME CYMFUSE*********************************************************')
    cymfuse = _readCymeFuse(net_db, feeder_id)
    logger.info('-8-CYME CYMRECLOSER*****************************************************')
    cymrecloser = _readCymeRecloser(net_db, feeder_id)
    logger.info('-9-CYME CYMREGULATOR****************************************************')
    cymregulator = _readCymeRegulator(net_db, feeder_id)
    logger.info('-10-CYME CYMSHUNTCAPACITOR**********************************************')
    cymshuntcapacitor = _readCymeShuntCapacitor(net_db, feeder_id, type)
    logger.info('-11-CYME CYMCUSTOMERLOAD************************************************')
    cymcustomerload = _readCymeCustomerLoad(net_db, feeder_id)
    logger.info('-12-CYME CYMSECTION ****************************************************')
    ...
    logger.info('-13-CYME CYMSECTIONDEVICE **********************************************')
    ...
    for link in cymsection.keys():
        ...
        if link_exists == 0:
            ...
            warnings.warn("There is no device associated with section:{:s} in network database:{:s}. This will be modeled as a switch.".format(
                link, net_db), RuntimeWarning)
    for link in cymsection.keys():
    ...
    for device in cymsectiondevice.keys():
        if cymsectiondevice[device]['device_type'] == 1:
            undergroundline_sections[
                cymsectiondevice[device]['section_name']] = device
    ...
    for x in [fuse_sections, regulator_sections, threewxfmr_sections, threewautoxfmr_sections, transformer_sections, switch_sections, recloser_sections, sectionalizer_sections]:
        if len(x) > 0:
            _splitLinkObjects(
                cymsection, cymsectiondevice, x, overheadline_sections, undergroundline_sections)
    logger.info('-14-CYME CYMTRANSFORMER*************************************************')
    cymxfmr = _readCymeTransformer(net_db, feeder_id)
    logger.info('-15-CYME CYMTHREEWINDINGTRANSFORMER*************************************')
    ...
    logger.info('-16-CYME CYMEQCONDUCTOR*************************************************')
    cymeqconductor = _readEqConductor(eqp_db, feeder_id)
    logger.info('-17-CYME CYMEQCONDUCTOR*************************************************')
    cymeqoverheadlineunbalanced = _readEqOverheadLineUnbalanced(
        eqp_db, feeder_id)
    logger.info('-17-CYME CYMEQGEOMETRICALARRANGEMENT************************************')
    if dbflag == 0:
        cymeqgeometricalarrangement = _readEqGeometricalArrangement(
            eqp_db, feeder_id)
    elif dbflag == 1:
        cymeqgeometricalarrangement = _readEqAvgGeometricalArrangement(
            eqp_db, feeder_id)
    logger.info('-18-CYME Read XLSX Sheet************************************************')
    cymcsvundergroundcable = _readUgConfiguration(
        conductor_data_csv, feeder_id)
    logger.info('-19-CYME CYMEQREGULATOR*************************************************')
    cymeqregulator = _readEqRegulator(eqp_db, feeder_id)
    logger.info('-20-CYME CYMEQTHREEWINDAUTOTRANSFORMER**********************************')
    cymeq3wautoxfmr = _readEqThreeWAutoXfmr(eqp_db, feeder_id)
    logger.info('-21-CYME CYMEQAUTOTRANSFORMER*******************************************')
    cymeqautoxfmr = _readEqAutoXfmr(eqp_db, feeder_id)
    logger.info('FINISHED READING FROM THE DATABASES*************************************')
    eqp_db.close()
    ...
    for link in cymsection.keys():
        if 'from' in cymsection[link].keys() and 'to' in cymsection[link].keys():
            if [cymsection[link]['from'], cymsection[link]['to']] in fromTo or [cymsection[link]['to'], cymsection[link]['from']] in fromTo:
                ...
                fromTo.append(
                    [cymsection[link]['from'], cymsection[link]['to']])
            if cymsection[link]['from'] not in fromNodes:
                fromNodes.append(cymsection[link]['from'])
            if cymsection[link]['to'] not in toNodes:
                toNodes.append(cymsection[link]['to'])
    ...
    for device in cymsectiondevice.keys():
        if 'parent' not in cymsectiondevice[device].keys():
            cymsectiondevice[device]['from'] = cymsection[
                cymsectiondevice[device]['section_name']]['from']
            cymsectiondevice[device]['to'] = cymsection[
                cymsectiondevice[device]['section_name']]['to']
            cymsectiondevice[device]['phases'] = cymsection[
                cymsectiondevice[device]['section_name']]['phases']
            cymsectiondevice[device]['fromLatitude'] = cymsection[
                cymsectiondevice[device]['section_name']]['fromX']
            cymsectiondevice[device]['fromLongitude'] = cymsection[
                cymsectiondevice[device]['section_name']]['fromY']
            cymsectiondevice[device]['toLatitude'] = cymsection[
                cymsectiondevice[device]['section_name']]['toX']
            cymsectiondevice[device]['toLongitude'] = cymsection[
                cymsectiondevice[device]['section_name']]['toY']
        # Create all the node dictionaries
            ...
            if cymsectiondevice[device]['parent'] not in nodes.keys() and cymsectiondevice[device]['parent'] != swingBus:
                ...
                if cymsectiondevice[device]['location'] == 2:
                    nodes[cymsectiondevice[device]['parent']][
                        'latitude'] = cymsectiondevice[device]['toLatitude']
                    nodes[cymsectiondevice[device]['parent']][
                        'longitude'] = cymsectiondevice[device]['toLongitude']
                else:
    ...
    for ols in uniqueOhSpacing:
        if ols in cymeqgeometricalarrangement.keys():
            ...
            logger.warning("There is no line spacing spec for %s in the equipment database provided.", ols)
    ...
    for olcfg in UOLConfigNames:
        if olcfg in cymeqoverheadlineunbalanced.keys():
            if olcfg not in ohl_cfgs.keys():
                ohl_cfgs[olcfg] = copy.deepcopy(
                    cymeqoverheadlineunbalanced[olcfg])
        else:
            logger.warning("There is no overhead line configuration for %s in the equipment database provided.", olcfg)
    ...
    for ulc in UG_conductors:
        if ulc in cymcsvundergroundcable.keys():
            ...
            logger.warning("Runtimerror: No configuratino spec for {:s} in the underground csv file provided.", ulc)
                    ...
                    for phase in ph:
                        if phase != 'D' or phase != 'N':
                            ugl_cfgs[config_name][
                                'conductor_{:s}'.format(phase)] = cymundergroundline[ugl]['cable_id'] + 'cond'
    ...
    for swObj in cymsectiondevice.keys():
        if cymsectiondevice[swObj]['device_type'] == 13:
            if swObj not in cymswitch.keys():
                logger.warning("There is no switch spec for %s in the network database provided.", swObj)
    ...
    for sxnlr in cymsectiondevice.keys():
        if cymsectiondevice[sxnlr]['device_type'] == 12:
            if sxnlr not in cymsectionalizer.keys():
                logger.warning("There is no sectionalizer spec for %s in the network database provided.", sxnlr)
    ...
    for fuse in cymsectiondevice.keys():
        if cymsectiondevice[fuse]['device_type'] == 14:
            if fuse not in cymfuse.keys():
                logger.warning("There is no fuse spec for %s in the network database provided.", fuse)
    ...
    for cap in cymsectiondevice.keys():
        if cymsectiondevice[cap]['device_type'] == 17:
            if cap not in cymshuntcapacitor.keys():
                logger.warning("There is no capacitor spec for %s in the network database provided.", cap)
                ...
                for phase in caps[cap]['phases']:
                    if phase not in ['N', 'D']:
                        ...
                        caps[cap]['capacitor_{:s}'.format(phase)] = str(
                            cymshuntcapacitor[cap]['capacitor_{:s}'.format(phase)])
    ...
    for load in cymsectiondevice.keys():
        if cymsectiondevice[load]['device_type'] == 20:
            if load not in cymcustomerload.keys():
                logger.warning("There is no load spec for %s in the network database provided.", load)
                ...
                for phase in loads[load]['phases']:
                    if phase not in ['N', 'D']:
                        loads[load]['constant_power_{:s}'.format(phase)] = cymcustomerload[load][
                            'constant_power_{:s}'.format(phase)]
            elif load not in tpns.keys():
                for phase in cymsectiondevice[load]['phases']:
                    if phase not in ['N', 'D']:
                        spctRating = _find_SPCT_rating(
                            cymcustomerload[load]['constant_power_{:s}'.format(phase)])
    ...
    for reg in cymsectiondevice.keys():
        if cymsectiondevice[reg]['device_type'] == 4:
            if reg not in cymregulator.keys():
                logger.warning("There is no regulator spec for %s in the network database provided.\n", reg)
            else:
                ...
                if regEq not in reg_cfgs.keys():
                    ...
                    for phase in ph:
                        reg_cfgs[regEq]['tap_pos_{:s}'.format(phase)] = str(
                            cymregulator[reg]['tap_pos_{:s}'.format(phase)])
    ...
    for xfmr in cymsectiondevice.keys():
        if cymsectiondevice[xfmr]['device_type'] == 47:
            if xfmr not in cymxfmr.keys():
                ...
                if xfmrEq not in cymeqautoxfmr.keys():
                    ...
                    if xfmrEq not in xfmr_cfgs.keys():
                        ...
                        for phase in ph:
                            if phase not in ['N', 'D']:
                                xfmr_cfgs[xfmrEq]['power{:s}_rating'.format(phase)] = '{:0.6f}'.format(
                                    cymeqautoxfmr[xfmrEq]['PrimaryRatedCapacity'] / phNum)
            ...
            if xfmr not in cym3wxfmr.keys():
                logger.warning("There is no xfmr spec for %s in the network database provided.",xfmr)
            else:
                ...
                if xfmrEq not in cymeq3wautoxfmr.keys():
                    ...
                    if xfmrEq not in xfmr_cfgs.keys():
                        ...
                        for phase in ph:
                            if phase not in ['N', 'D']:
                                xfmr_cfgs[xfmrEq]['power{:s}_rating'.format(phase)] = '{:0.6f}'.format(
                                    cymeq3wautoxfmr[xfmrEq]['PrimaryRatedCapacity'] / phNum)
                if xfmr not in xfmrs.keys():
                    xfmrs[xfmr] = {'object': 'transformer',
                                   'name': xfmr,
                                   'phases': ph,
    ...
    key = len(glmTree)
    objectList = [ohl_conds, ugl_conds, ohl_spcs, ugl_sps, ohl_cfgs, ugl_cfgs, xfmr_cfgs, spct_cfgs, reg_cfgs,
                  meters, nodes, loads, tpms, tpns, ohls, ugls, xfmrs, spcts, regs, swObjs, rcls, sxnlrs, fuses, caps]
    ...
    for key in glmTree.keys():
        # if ('from' in glmTree[key].keys() and 'to' not in
        # glmTree[key].keys()) or ('to' in glmTree[key].keys() and 'from' not
        # in glmTree[key].keys()):
        ...
        for x in glm_dict.keys():
            if 'from' in glm_dict[x].keys() and glm_dict[x]['from'] in volt_dict.keys() and glm_dict[x]['to'] not in volt_dict.keys():
                if glm_dict[x]['object'] == 'transformer':
                    ...
                    volt_dict[glm_dict[x]['to']] = volt_dict[
                        glm_dict[x]['from']]
                    cnfg = glm_dict[x]['configuration']
                    ...
                    for y in glm_dict.keys():
                        if glm_dict[y].get('name', '') == cnfg:
                            ...
                            glmTree[y]['band_width'] = str(
                                float(glmTree[y]['band_width']) * float(glmTree[y]['band_center']))
                else:
                    volt_dict[glm_dict[x]['to']] = volt_dict[
                        glm_dict[x]['from']]
            elif 'parent' in glm_dict[x].keys() and glm_dict[x]['parent'] in volt_dict.keys() and glm_dict[x]['name'] not in volt_dict.keys():
                volt_dict[glm_dict[x]['name']] = volt_dict[
                    glm_dict[x]['parent']]
    ...
    for obj in glmTree:
        if 'bustype' in glmTree[obj] and glmTree[obj]['bustype'] == 'SWING':
            parent_voltage[glmTree[obj]['name']] = glmTree[
                obj]['nominal_voltage']
            current_parents = len(parent_voltage)
    ...
    for x in glmTree.keys():
        if glmTree[x].get('name', '') in parent_voltage.keys():
            glmTree[x]['nominal_voltage'] = parent_voltage[
                glmTree[x].get('name', '')]
    # Delete nominal_voltage from link objects
    del_nom_volt_list = ['overhead_line', 'underground_line',
                         'regulator', 'transformer', 'switch', 'fuse', 'ZIPload', 'diesel_dg']
    for x in glmTree:
...
def _tests(keepFiles=True):
    ...
    try:
        ...
        cyme_base, x, y = convertCymeModel(
            db_network, db_equipment, test=False, type=1, feeder_id='650')
        ...
        try:
            ...
            Plot.save_fig(plt, outPrefix + "IEEE13.png")
            print 'DREW GLM OF'
        except:
            exceptionCount += 1
