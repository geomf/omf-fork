...
def run(modelDir, inputDict, fs):
    ...
    try:
        ...
        ssc.ssc_data_set_number(dat, "derate", float(inputDict.get(
            "inverterEfficiency", 96)) / 100 * float(inputDict.get("nonInverterEfficiency", 87)) / 100)
        ssc.ssc_data_set_number(dat, "track_mode", float(trackingMode))
        ssc.ssc_data_set_number(
            dat, "azimuth", float(inputDict.get("azimuth", 180)))
        ...
        # Geodata output.
        # Geodata output.
        outData["minLandSize"] = round((arraySizeDC / 1390.8 * 5 + 1) * math.cos(
            math.radians(22.5)) / math.cos(math.radians(latforpvwatts)), 0)
        ...
        outData["powerOutputAcInvClipped"] = [
            x if x < invSizeWatts else invSizeWatts for x in outData["powerOutputAc"]]
        try:
        ...
        # One year generation
        outData["oneYearGenerationWh"] = sum(
            outData["powerOutputAcInvClipped"])
        ...
        for i in range(2, loanYears + 1):
            outData["allYearGenerationMWh"][i] = float(outData["allYearGenerationMWh"][
                                                       i - 1]) * (1 - float(inputDict.get("degradation", 0.8)) / 100)
        ...
        ######
        # Hardware Costs
        pvModules = arraySizeDC * \
            float(inputDict.get("moduleCost", 0)) * 1000  # off by 4000
        ...
        hardwareCosts = (pvModules + racking + inverters + gear + balance +
                         combiners + wireManagement + transformer + weatherStation) * shipping
        ...
        laborDesign = designLabor + electricalLabor + \
            float(inputDict.get("pmCost", 0)) + EPCmarkup
        materialDesign = 0
        ...
        siteMaterial = 8000 + 600 + 5500 + 5000 + surveying + \
            concrete + fencing + grading + landscaping + 5600
        ...
        # Construction Costs (Office Trailer, Skid Steer, Storage Containers,
        # etc)
        ...
        installCosts = moduleAndRackingInstall + pierDriving + balanceInstall + \
            float(inputDict.get("elecLabor", 0)) * \
            (72 + 60 + 70 + 10 + 5 + 30 + 70)
        ...
        # Total Costs
        ######
        totalCosts = hardwareCosts + designCosts + \
            sitePrep + constrEquip + installCosts + landCosts
        totalFees = float(inputDict.get("devCost", 0)) / 100 * totalCosts
        outData["totalCost"] = totalCosts + totalFees + \
            float(inputDict.get("interCost", 0))
        ...
        outData["costsPieChart"] = [["Land", landCosts],
                                    ...
                                    ["BOS", hardwareCosts - pvModules * shipping -
                                        racking * shipping - (inverters + gear) * shipping],
                                    ["Site Prep, Constr. Eq. and Installation", (siteMaterial + constrEquip) + (siteLabor + installCosts)]]

        # Cost per Wdc
        outData["costWdc"] = (
            totalCosts + totalFees + float(inputDict.get("interCost", 0))) / (arraySizeDC * 1000)

        outData["capFactor"] = float(
            outData["oneYearGenerationWh"]) / (inverterSizeAC * 1000 * 365.25 * 24) * 100
        ...
        for i in range(0, 12):
            ...
            leasePaymentsLease.append(-1 * projectCostsLease /
                                      ((1.0 - (1.0 / (1.0 + leaseRate)**12)) / (leaseRate)))
        ...
        #Output - Lease [G]	[H]
        for i in range(1, len(outData["allYearGenerationMWh"]) + 1):
            netCoopPaymentsLease.append(
                OMInsuranceETCLease[i - 1] + leasePaymentsLease[i - 1])
            costToCustomerLease.append(
                netCoopPaymentsLease[i - 1] - distAdderLease[i - 1])

        # Output - Lease [H44]. Note the extra year at the zero point to get
        # the discounting right.
        NPVLease = npv(
            float(inputDict.get("discRate", 0)) / 100, [0, 0] + costToCustomerLease)
        ...
        outData["cost10WPanelTaxLease"] = (
            float(outData["costPanelTaxLease"]) / float(panelSize)) * 10
        ...
        for p in range(0, 3):
            while ((z < 50000) and (nValue < nGoal)):
                achievedReturnTE, Rate_Levelized_TaxEquity, NPVLoanTaxEquity = taxEquityFlip(PPARateSixYearsTE, inputDict.get("discRate", 0), outData["totalCost"], outData[
                                                                                             "allYearGenerationMWh"], distAdderDirect, loanYears, firstYearLandLeaseCosts, firstYearOPMainCosts, firstYearInsuranceCosts, numberPanels)
        ...
        outData["cost10WPanelTaxEquity"] = (
            float(outData["costPanelTaxEquity"]) / panelSize) * 10
        ...
        #Output - PPA [G] [H]
        for i in range(1, len(outData["allYearGenerationMWh"]) + 1):
            netCoopPaymentsPPA.append(-outData["allYearGenerationMWh"][i] * float(inputDict.get(
                "firstYearEnergyCostPPA", 0)) * math.pow((1 + float(inputDict.get("annualEscRatePPA", 0)) / 100), (i - 1)))
            costToCustomerPPA.append(
                netCoopPaymentsPPA[i - 1] - distAdderPPA[i - 1])
