...
#		return _dll
    ...
    @staticmethod
    def ssc_data_create():
        logger.info('Creating NREL SAM Data Object...')
    ...
    def ssc_data_set_string(p_data, name, value):
        logger.debug('Adding string to NREL SAM Data Object... name: %s; value: %s', name, value)
        SSCAPI._dll.ssc_data_set_string(
            c_void_p(p_data), c_char_p(name), c_char_p(value))

    @staticmethod
    def ssc_data_set_number(p_data, name, value):
        logger.debug('Adding number to NREL SAM Data Object... name: %s; value: %s', name, value)
        SSCAPI._dll.ssc_data_set_number(
            c_void_p(p_data), c_char_p(name), c_number(value))
    ...
    def ssc_data_set_table(p_data, name, table):
        SSCAPI._dll.ssc_data_set_table(
            c_void_p(p_data), c_char_p(name), c_void_p(table))

    @staticmethod
    def ssc_data_get_string(p_data, name):
        SSCAPI._dll.ssc_data_get_string.restype = c_char_p
        value = SSCAPI._dll.ssc_data_get_string(c_void_p(p_data), c_char_p(name))
        logger.debug('Got string from NREL SAM Data Object... name: %s; value: %s', name, value)
        return value

    @staticmethod
    def ssc_data_get_number(p_data, name):
        val = c_number(0)
        SSCAPI._dll.ssc_data_get_number(
            c_void_p(p_data), c_char_p(name), byref(val))
        logger.debug('Got number from NREL SAM Data Object... name: %s; value: %s', name, val.value)
        return val.value
    ...
    def ssc_data_get_array(p_data, name):
        ...
        parr = SSCAPI._dll.ssc_data_get_array(
            c_void_p(p_data), c_char_p(name), byref(count))
        arr = []
        for i in range(count.value):
            arr.append(float(parr[i]))
        logger.debug('Got array from NREL SAM Data Object... name: %s; length: %s', name, arr.__len__())
        return arr
    ...
    def ssc_data_get_matrix(p_data, name):
        ...
        parr = SSCAPI._dll.ssc_data_get_matrix(
    ...
    def ssc_module_create(name):
        logger.info('Creating NREL SAM Module... name: %s', name)
    ...
    def ssc_module_exec(p_mod, p_data):
        logger.info('Executing NREL SAM Simulation Core...')
        start = datetime.datetime.utcnow()
        SSCAPI._dll.ssc_module_exec.restype = c_int
        ret = SSCAPI._dll.ssc_module_exec(c_void_p(p_mod), c_void_p(p_data))
        logger.info('NREL SAM Simulation Core finished in %s. Exit code: %s',
                    datetime.datetime.utcnow() - start, ret)
        return ret
...
class API:
    ...
    # constants for return value of Data.Query() and Info.DataType() (see
    # sscapi.h)
    INVALID = 0
    STRING = 1
    NUMBER = 2
...
class Info:
    ...
    def get(self):
        self._inf = SSCAPI.ssc_module_var_info(
            self._mod.get_module_handle(), self._idx)
